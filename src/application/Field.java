...
public class Field {
	...
	private int countTileNeighbours(Tile tile) {
		...
		for (int i = -1; i <= 1; i++) {
			for (int j = -1; j <= 1; j++) {
				if (j != 0 || i != 0) {
					if (tile.getX() + j >= 0 && tile.getX() + j < width && tile.getY() + i >= 0
							&& tile.getY() + i < height) {
	...
	public Tile[][] mineLayer(int mines) {
		...
		if (mines > height * width) {
			...
			while (minesLeft > 0) {
				// slumpa x och y-led med planens height och width som
				// constraints
				int currentX = randInt(0, width - 1); // Slumpa mellan 0 och
														// width
				int currentY = randInt(0, height - 1); // Slumpa mellan 0 och
														// Height
